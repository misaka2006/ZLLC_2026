...
void Class_Chariot::CAN_Chassis_Tx_Gimbal_Callback()
{
    ...
    // 发送数据给云台
    CAN3_Chassis_Tx_Data_A[0] = Referee.Get_ID();
    CAN3_Chassis_Tx_Data_A[1] = Referee.Get_Game_Stage();
    memcpy(CAN3_Chassis_Tx_Data_A + 2, &Shooter_Barrel_Heat_Limit, sizeof(uint16_t));
    memcpy(CAN3_Chassis_Tx_Data_A + 4, &Shooter_Cool, sizeof(uint16_t));
    memcpy(CAN3_Chassis_Tx_Data_A + 6, &Shooter_Speed, sizeof(uint16_t));
}
#endif
...
void Class_Chariot::Control_Chassis()
{
    ...
    if (MiniPC.Get_MiniPC_Status() != MiniPC_Status_DISABLE && DR16.Get_Left_Switch() == DR16_Switch_Status_DOWN)
    { // 上位机导航信息接收
    ...
    { // 随动 附有非随动和受击陀螺逻辑
        if (Gimbal.Motor_Yaw.Get_DJI_Motor_Status() == LK_Motor_Status_DISABLE)
        { // 大yaw离线失能
...
void Class_Chariot::Control_Gimbal()
{
    ...
    if (tmp_gimbal_pitch > 18.0f)
        tmp_gimbal_pitch = 18.0f;
