...
void Class_Chariot::Init(float __DR16_Dead_Zone)
{
    ...
    Motor_Yaw.Init(&hfdcan2, DJI_Motor_ID_0x206);
...
/**
 * @brief 判断VT13控制器的控制类型
 *
 * 该函数根据VT13控制器的输入状态来判断当前的控制类型：
 * 1. 如果左右摇杆有输入，则判定为遥控器控制
 * 2. 如果鼠标或键盘有输入，则判定为键盘控制
 * 3. 如果VT13控制器被禁用，则判定为无控制
 */
