 ...
 * @param pos 正解算位置 [x, y, z]
 * @param rpy 正解算姿态 [yaw, pitch, roll]
 * @return bool 是否成功
 */
...
bool Class_DH_model::Fkine(float joint_angles[6], float T[4][4])
{
    ...
    Matrixf<4, 4> T_mat = robot.fkine(q); // 写入旋转矩阵
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 4; j++)
        {
...
{
    float q_solutions[8][6];
    bool valid[8];
    float distance;

    uint8_t valid_cnt = Ikine_Pieper(pos_target, rpy_target, q_solutions, valid);
    ...
    else
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            d[i] = Euclidean_Distance(q_start, q_solutions[i]);
        ...
        memcpy(q_solution, q_solutions[best_index], 6 * sizeof(float));
    }
...
float Class_Trajectory_Tracer::normalize_angle(float angle)
{
    // 使用 user_lib.h 中的宏
